package join_patterns.matching.eager_removal_filtering
...
class EagerRemovalFilteringMatchingTree[M, T](private val pattern: JoinPattern[M, T], private val patternIdx: Int):
          ...
          bins.get(matchingConstructorIdxs) match
            ...
            case Some(mappedMessageIdxs) =>
              ...
              if mappedMessageIdxs.size < matchingConstructorIdxs.size then
                ...
                val updatedFilters = ArrayBuffer[Int]()
                val newAdvancedFilterState = advancedFilterState.zipWithIndex.map: (statesAndFilter, i) =>
                  val (states, filter) = statesAndFilter
                  val newStates = states.updatedWith(matchingConstructorIdxs):
                    case Some(false) =>
                      updatedFilters.append(i)
                      Some(true)
                    case None => None
                ...
                val filtersToRemove = ArrayBuffer[Int]()
                val allFiltersSatisfied = updatedFilters.forall: i =>
                  val completionState = newAdvancedFilterState(i)._1

                  if completionState.exists((pidxs, filled) => !filled) then true
                  else
                    val filterToCheck = newAdvancedFilterState(i)._2

                    val lookupEnvForFilter =
                      completionState.foldLeft(LookupEnv.empty): (acc, state) =>
                        val (pidxs, _) = state
                    ...
                    val satisfied = filterToCheck(lookupEnvForFilter)
                    if satisfied then filtersToRemove.append(i)
                    satisfied
                ...
                then
                  ...
                  val finalAdvancedFilterState = newAdvancedFilterState.iterator
                    .zipWithIndex
                    .filterNot((e, i) => filtersToRemove.contains(i))
                    .map((e, i) => e)
                    .to(ArraySeq)
                  val newNode = (newMessageIdxs, (newPatternBins, finalAdvancedFilterState))
...
object EagerRemovalFilteringMatchingTree:
