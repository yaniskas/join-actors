...
class EagerRemovalFilteringMatcher[M, T](private val patterns: JoinDefinition[M, T])
  extends Matcher[M, T], MutableMapMessageStore[M]:

  private var nextMessageIndex = 0

  private val matchingTrees: List[EagerRemovalFilteringMatchingTree[M, T]] =
    patterns.zipWithIndex.map(EagerRemovalFilteringMatchingTree(_, _))
...
object EagerRemovalFilteringMatcher extends MatcherFactory:
  def apply[M, T]: JoinDefinition[M, T] => Matcher[M, T] =
    (joinDefinition: JoinDefinition[M, T]) => new EagerRemovalFilteringMatcher(joinDefinition)

  override def toString: String = "EagerRemovalFilteringMatcher"
