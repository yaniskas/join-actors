...
class SimpleSmartHouse(private val matcher: MatcherFactory, private val config: Config)
    extends MessageFeedBenchmark[Action]:
  override def prepare(n: Int): MessageFeedTriplet[Action] =
    ...
    val (result, ref) = getSmartHouseActor(matcher, config.withHeavyGuard).start()

    (result, ref, msgs)

  private def getSmartHouseActor(matcher: MatcherFactory, withHeavyGuard: Boolean) =
...
object SimpleSmartHouse extends BenchmarkFactory:
  override def apply(matcher: MatcherFactory, config: SimpleSmartHouseConfig): InstanceType =
    new SimpleSmartHouse(matcher, config)
