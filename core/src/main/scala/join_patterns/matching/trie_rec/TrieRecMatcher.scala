...
class TrieRecMatcher[M, T](private val patterns: JoinDefinition[M, T])
  extends Matcher[M, T], MutableMapMessageStore[M]:

  private var nextMessageIndex = 0

  private val matchingTrees: List[TrieRecMatchingTree[M, T]] =
    patterns.zipWithIndex.map(TrieRecMatchingTree(_, _))
...
object TrieRecMatcher extends MatcherFactory:
  def apply[M, T]: JoinDefinition[M, T] => Matcher[M, T] =
    (joinDefinition: JoinDefinition[M, T]) => new TrieRecMatcher(joinDefinition)

  override def toString: String = "TrieRecMatcher"
