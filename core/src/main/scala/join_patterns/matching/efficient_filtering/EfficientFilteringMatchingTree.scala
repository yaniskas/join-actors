package join_patterns.matching.efficient_filtering
...
class EfficientFilteringMatchingTree[M, T](private val pattern: JoinPattern[M, T], private val patternIdx: Int):
              ...
              if mappedMessageIdxs.size < matchingConstructorIdxs.size then
                ...
                val allFiltersSatisfied = updatedFilters.forall: i =>
                  ...
                  if completionState.exists((pidxs, filled) => !filled) then true
                    ...
                    filterToCheck(lookupEnvForFilter)
                ...
                then
                  ...
                  val newNode = (newMessageIdxs, (newPatternBins, newAdvancedFilterState))
...
object EfficientFilteringMatchingTree:
