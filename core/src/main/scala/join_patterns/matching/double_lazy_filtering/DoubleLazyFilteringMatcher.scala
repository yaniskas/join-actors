...
class DoubleLazyFilteringMatcher[M, T](private val patterns: JoinDefinition[M, T])
  extends Matcher[M, T], MutableMapMessageStore[M]:

  private var nextMessageIndex = 0

  private val matchingTrees: List[DoubleLazyFilteringMatchingTree[M, T]] =
    patterns.zipWithIndex.map(DoubleLazyFilteringMatchingTree(_, _))
...
object DoubleLazyFilteringMatcher extends MatcherFactory:
  def apply[M, T]: JoinDefinition[M, T] => Matcher[M, T] =
    (joinDefinition: JoinDefinition[M, T]) => new DoubleLazyFilteringMatcher(joinDefinition)

  override def toString: String = "DoubleLazyFilteringMatcher"
