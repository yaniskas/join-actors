...
class ComplexSmartHouse(private val matcher: MatcherFactory, private val config: Config)
  ...
  override def prepare(n: Int): MessageFeedTriplet[Action] =
    ...
    val (result, ref) = getSmartHouseActor(matcher).start()

    (result, ref, msgs.to(ArraySeq))

  private def getSmartHouseActor(matcher: MatcherFactory): Actor[Action, (Long, Int)] =
...
object ComplexSmartHouse extends BenchmarkFactory:
  override def apply(matcher: MatcherFactory, config: ComplexSmartHouseConfig): InstanceType =
    new ComplexSmartHouse(matcher, config)
