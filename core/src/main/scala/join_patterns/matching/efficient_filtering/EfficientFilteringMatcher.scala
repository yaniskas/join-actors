...
class EfficientFilteringMatcher[M, T](private val patterns: JoinDefinition[M, T])
  extends Matcher[M, T], MutableMapMessageStore[M]:

  private var nextMessageIndex = 0

  private val matchingTrees: List[EfficientFilteringMatchingTree[M, T]] =
    patterns.zipWithIndex.map(EfficientFilteringMatchingTree(_, _))
...
object EfficientFilteringMatcher extends MatcherFactory:
  def apply[M, T]: JoinDefinition[M, T] => Matcher[M, T] =
    (joinDefinition: JoinDefinition[M, T]) => new EfficientFilteringMatcher(joinDefinition)

  override def toString: String = "EfficientFilteringMatcher"
