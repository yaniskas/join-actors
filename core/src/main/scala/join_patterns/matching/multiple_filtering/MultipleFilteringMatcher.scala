...
class MultipleFilteringMatcher[M, T](private val patterns: JoinDefinition[M, T])
  extends Matcher[M, T], MutableMapMessageStore[M]:

  private var nextMessageIndex = 0

  private val matchingTrees: List[MultipleFilteringMatchingTree[M, T]] =
    patterns.zipWithIndex.map(MultipleFilteringMatchingTree(_, _))
...
object MultipleFilteringMatcher extends MatcherFactory:
